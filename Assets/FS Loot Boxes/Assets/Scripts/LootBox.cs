...
public class LootBox : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (player != null)
        {
            ...
            isPlayerAround = distance <= detectionRange;

            if (isPlayerAround && openingMethod == OpeningMethods.OpenOnKeyPress)
            {
                if (Input.GetKey(keyCode)) Open();
            }

            // when player leave far enough to close the box
            if (!isPlayerAround && closeOnExit && isOpen)
            {
                Close();
            }
        }
